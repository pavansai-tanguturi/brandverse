...
export async function adminAddItem(req, res) {
  ...
  // log event
  await supabaseAdmin.from('events').insert({ type: 'admin_add_cart_item', customer_id: null, product_id, metadata: { cart_id, admin: req.user.id, quantity } });
...
export async function adminRemoveItem(req, res) {
  ...
  if (error) return res.status(400).json({ error: error.message });
  await supabaseAdmin.from('events').insert({ type: 'admin_remove_cart_item', customer_id: null, product_id: item.product_id, metadata: { cart_id: item.cart_id, admin: req.user.id, quantity: item.quantity } });
