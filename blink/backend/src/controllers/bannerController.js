...
export const createBanner = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create banner", details: err.message });
  }
};
...
export const updateBanner = async (req, res) => {
  try {
    ...
    if (!req.file) {
      console.log(
        "--- DEBUG: [PATCH] No file received in req.file. Checking if image_url is present in form data:",
        imageUrl,
      );
      if (!imageUrl) {
        ...
        if (fetchError) {
          console.error(
            "--- DEBUG: [PATCH] Error fetching current banner for image_url:",
            fetchError,
          );
          throw fetchError;
        }
        ...
        console.log(
          "--- DEBUG: [PATCH] Using existing image_url from DB:",
          imageUrl,
        );
      }
    }
    if (req.file) {
      ...
      const fileName = `banner_${Date.now()}.${fileExt}`;
      console.log(
        "--- DEBUG: [PATCH] Uploading to bucket:",
        BUCKET,
        "as",
        fileName,
      );
      ...
      if (uploadResult.error) {
        console.error(
          "--- DEBUG: [PATCH] Supabase upload error:",
          uploadResult.error,
        );
        throw uploadResult.error;
      }
      const publicUrlResult = supabaseAdmin.storage
        ...
        .getPublicUrl(fileName);
      console.log(
        "--- DEBUG: [PATCH] Supabase getPublicUrl result:",
        publicUrlResult,
      );
      imageUrl = publicUrlResult.data.publicUrl;
    ...
    res
      .status(500)
      .json({ error: "Failed to update banner", details: err.message });
  }
};
...
export const deleteBanner = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete banner", details: err.message });
  }
};
