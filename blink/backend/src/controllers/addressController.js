...
const validateAddressData = (addressData) => {
  ...
  if (
    addressData.phone &&
    !/^\d{10}$/.test(addressData.phone.replace(/[-\s]/g, ""))
  ) {
  ...
  try {
    ...
    if (!full_name || !address_line_1 || !city || !state || !postal_code) {
      return res.status(400).json({
        error:
          "Required fields: full_name, address_line_1, city, state, postal_code",
      });
    }
...
const getCurrentUserAddresses = async (req, res) => {
  try {
    ...
    if (customerError || !customer) {
      console.error(
        "Customer not found for auth_user_id:",
        auth_user_id,
        customerError,
      );
...
const createCurrentUserAddress = async (req, res) => {
  try {
    ...
    if (customerError || !customer) {
      ...
      console.log("Attempting to create customer record for:", {
        auth_user_id,
        userEmail,
        userName,
      });
      ...
      if (createError || !newCustomer) {
        ...
        return res
          .status(500)
          .json({
            error: "Failed to create customer record. Please contact support.",
          });
      }
    ...
    const customer_id = customer.id;
    console.log("Found customer for address creation:", {
      auth_user_id,
      customer_id,
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Required fields: full_name, phone, address_line_1, city, state, postal_code",
      });
    }
  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (!isOwner) {
        return res
          .status(403)
          .json({ error: "Not authorized to delete one or more addresses" });
      }
    }
...
const duplicateAddress = async (req, res) => {
  try {
    ...
    if (!isOwner) {
      return res
        .status(403)
        .json({ error: "Not authorized to access this address" });
    }
