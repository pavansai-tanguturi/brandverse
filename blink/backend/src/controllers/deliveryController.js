  ...
  // Use the working auth from your controller instead of middleware
  ...
  export async function getDeliveryLocations(req, res) {
    try {
      ...
      if (error) {
        console.log('Delivery locations table not found or error:', error.message);
        return res.json({ 
          deliveryLocations: [],
  ...
  export async function checkDeliveryAvailability(req, res) {
    try {
      ...
      console.log(`🔍 Checking delivery for: ${city || 'N/A'}, ${region || 'N/A'}, ${country}`);
      ...
      try {
        ...
        if (city && region) {
          ...
          if (!exactError && exactMatch && exactMatch.length > 0) {
            ...
            const locationName = [location.city, location.region, location.country].filter(Boolean).join(', ');
        ...
        if (region) {
          ...
          if (!regionError && regionMatch && regionMatch.length > 0) {
            ...
            const requestedLocation = [city, region, country].filter(Boolean).join(', ');
            console.log(`✅ Region-wide delivery found for: ${requestedLocation}`);
        ...
        if (isAvailable) {
          ...
          const { data: disabledLocation, error: disabledError } = await supabaseAdmin
            .from('delivery_locations')
            .select('*')
            .eq('is_active', false)
            .ilike('country', country)
        ...
        return res.json({ 
          available: false, 
          location: null,
          message: 'Unable to check delivery availability at this time. Please try again later.',
  ...
  export async function getDeliveryStats(req, res) {
    try {
      ...
      const countries = [...new Set(stats?.map(loc => loc.country) || [])].length;
      const regions = [...new Set(stats?.filter(loc => loc.region).map(loc => loc.region) || [])].length;
      const cities = [...new Set(stats?.filter(loc => loc.city).map(loc => loc.city) || [])].length;
  ...
  export async function adminGetDeliveryLocations(req, res) {
    try {
      ...
      let query = supabaseAdmin.from('delivery_locations').select('*', { count: 'exact' });

      // Apply filters
      if (search) {
        query = query.or(`country.ilike.%${search}%,region.ilike.%${search}%,city.ilike.%${search}%`);
      }
  ...
  export async function adminAddDeliveryLocation(req, res) {
    try {
      ...
      if (existing && existing.length > 0) {
        return res.status(409).json({ error: 'This delivery location already exists' });
      }
      ...
      locations.forEach((loc, index) => {
        ...
        if (errors.length > 0) {
          ...
          validLocations.push({
            ...
            is_active: Boolean(loc.is_active !== undefined ? loc.is_active : true)
          });
        }
      });
  ...
  // Enhanced adminBulkDeleteDeliveryLocations with debug logging
  export async function adminBulkDeleteDeliveryLocations(req, res) {
    ...
    try {
      ...
      console.log('ids length:', ids ? ids.length : 'undefined');

      // Check if req.body exists at all
      if (!req.body) {
        console.log('ERROR: req.body is undefined - missing body parser middleware?');
        return res.status(400).json({ error: 'Request body is missing' });
      }

      // Check if ids exists
      if (!ids) {
        console.log('ERROR: ids field is missing from request body');
        return res.status(400).json({ error: 'ids field is required in request body' });
      }

      // Check if ids is an array
      if (!Array.isArray(ids)) {
        console.log('ERROR: ids is not an array, it is:', typeof ids);
        return res.status(400).json({ error: 'ids must be an array' });
      }

      // Check if array is empty
      if (ids.length === 0) {
        console.log('ERROR: ids array is empty');
        return res.status(400).json({ error: 'ids array cannot be empty' });
      }

      console.log('Validation passed, proceeding with delete for IDs:', ids);
      ...
      console.log('Supabase delete result - data:', data);
      ...
      console.log('=== BULK DELETE DEBUG END ===');
      ...
      console.error('=== BULK DELETE CATCH ERROR ===');
  ...
  export async function adminExportDeliveryLocations(req, res) {
    try {
      ...
      if (format === 'csv') {
        ...
        const headers = ['ID', 'Country', 'Region', 'City', 'Active', 'Created At', 'Updated At'];
        const csvData = [
          headers.join(','),
          ...data.map(loc => [
            loc.id,
        ...
        res.setHeader('Content-Disposition', `attachment; filename="delivery_locations_${new Date().toISOString().split('T')[0]}.csv"`);
        res.send(csvData);
      } else {
        res.json({
          deliveryLocations: data || [],
  ...
  // Get a single delivery location by ID (admin)
export async function adminGetDeliveryLocation(req, res) {
  try {
    const { id } = req.params;
