  ...
  try {
    ...
    if (itemsError || !items) {
      ...
      const { data: products, error: productsError } = await supabase
        .from("products")
        .select(
          "id, title, price_cents, stock_quantity, is_active, discount_percent",
        )
        .in("id", productIds);
      ...
      items = cartItems.map((cartItem) => {
        ...
        if (productWithImage) {
          const primaryImage = images?.find(
            (img) => img.product_id === productWithImage.id,
          );
    ...
    const validItems =
      items?.filter((item) => item.products && item.products.is_active) || [];
        ...
        item.products.discount_percent > 0
          ? Math.round(
              item.products.price_cents *
                (1 - item.products.discount_percent / 100),
            )
          : item.products.price_cents;
...
export async function addItem(req, res) {
  try {
    ...
    if (!product_id || !quantity || quantity < 1) {
      return res
        .status(400)
        .json({ error: "Valid product ID and quantity required" });
    }
    ...
    if (existingItem) {
      ...
      console.log(
        "DEBUG existingItem.id:",
        existingItem.id,
        "type:",
        typeof existingItem.id,
      );
      ...
      if (error) {
        ...
        return res
          ...
          .json({
            ...
            debug: { existingItem, id: existingItem.id, error },
          });
      }
...
export async function updateCartQuantity(req, res) {
  try {
    ...
    if (!Number.isInteger(quantity) || quantity < 1) {
      return res
        .status(400)
        .json({ error: "Valid quantity required (minimum 1)" });
    }

    const userId = req.user.id;
    console.log(
      `Updating cart quantity for user ${userId}, product ${product_id}, quantity ${quantity}`,
    );
