    ...
    if (format === 'json') {
      ...
      res.json({
        ...
        summary: {
          ...
          totalRevenue: ((orders || []).reduce((sum, order) => sum + (order.total_cents || 0), 0) / 100),
      ...
      const csvHeader = 'Order ID,Customer Name,Customer Email,Total,Status,Date,Items\n';
      const csvRows = (orders || []).map(order => {
        ...
        const items = (order.order_items || []).map(item => `${item.title}(${item.quantity})`).join(';');
        const date = dayjs(order.created_at).format('YYYY-MM-DD');
