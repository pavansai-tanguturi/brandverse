...
async function runDeliveryLocationsMigration() {
  try {
    ...
    const migrationPath = path.join(process.cwd(), 'sql', '008_create_delivery_locations.sql');
    ...
    console.log('Please run the following SQL in your Supabase SQL editor:');
    try {
      const migrationSQL = fs.readFileSync(path.join(process.cwd(), 'sql', '008_create_delivery_locations.sql'), 'utf8');
      console.log('\n' + migrationSQL);
    } catch (readError) {
