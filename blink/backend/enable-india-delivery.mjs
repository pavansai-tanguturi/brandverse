  ...
  try {
    ...
    if (countryWideEntry && countryWideEntry.length > 0) {
      ...
      if (entry.is_active) {
        ...
        console.log('   This means ALL locations in India should be supported!');
      } else {
      ...
      if (insertError) {
        console.error('❌ Failed to create country-wide India entry:', insertError);
      } else {
        console.log('✅ Country-wide India delivery entry created and ENABLED!');
      }
    }
    ...
    for (const testLocation of testLocations) {
      ...
      if (isAvailable) {
        ...
        const matchType = location.city ? 'city-specific' : location.region ? 'region-specific' : 'country-wide';
        console.log(`   ✅ DELIVERY AVAILABLE (${matchType})`);
        console.log(`   📍 Matched: ${[location.city, location.region, location.country].filter(Boolean).join(', ')}`);
      } else {
        ...
        if (countryMatch && countryMatch.length > 0) {
          console.log(`   💡 Should be available due to country-wide delivery!`);
        }
      }
    }
