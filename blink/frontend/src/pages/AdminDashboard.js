...
const transformApiData = (data) => {
  ...
  return {
    ...
    orderStatus: Array.isArray(orderStatus) ? orderStatus.map(status => ({
      ...status,
...
// Component parts
const MetricCard = React.memo(({ title, value, subtitle, color, icon, gradient }) => (
...
const QuickActionCard = React.memo(({ action }) => (
  ...
  >
    <div className="mb-3 group-hover:scale-110 transition-transform flex justify-center">
      <svg className={`w-10 h-10 text-${action.color}-600`} fill="currentColor" viewBox="0 0 20 20">
        <path d={action.icon} />
      </svg>
    </div>
...
const AdminDashboard = () => {
  ...
  const additionalMetrics = useMemo(() => [
    { label: "Monthly Revenue", value: formatCurrency(summary.monthlyRevenue), color: "gray" },
    { label: "Pending Orders", value: summary.pendingOrders, color: "orange" },
    { label: "Active Products", value: summary.activeProducts, color: "green" },
