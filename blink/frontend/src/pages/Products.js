...
function Products() {
  ...
  useEffect(() => {
    ...
    const hasMaxPrice =
      typeof urlMaxPrice === "number" &&
      !Number.isNaN(urlMaxPrice) &&
      urlMaxPrice > 0;
    ...
    filtered = filtered.filter((product) => {
      ...
      const upper = hasMaxPrice
        ? Math.min(priceRange[1], urlMaxPrice)
        : priceRange[1];
    ...
    const minDiscount = Math.max(
      0,
      urlDiscount,
      urlTag === "great-deal" ? 30 : 0,
    );
    if (minDiscount > 0) {
      filtered = filtered.filter(
        (p) => (p.discount_percent || 0) >= minDiscount,
      );
    }
  ...
  useEffect(() => {
    ...
    if (
      ["price-low", "price-high", "discount", "name"].includes(urlSort) &&
      urlSort !== sortBy
    ) {
      setSortBy(urlSort);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 pb-20 lg:pb-0">
      ...
      {/* Filter Section - Desktop: Horizontal, Mobile: Filter Button */}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="md:hidden flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg shadow-sm hover:shadow-md transition-all"
              >
                <svg
      ...
      {isMobileFilterOpen && (
        <div className="fixed inset-0 z-[60] md:hidden">
          {/* Backdrop */}
          <div
            className="fixed inset-0 backdrop-blur-md bg-black/30 saturate-150"
          ...
          <div className="fixed inset-x-0 bottom-0 bg-white rounded-t-xl max-h-[80vh] overflow-y-auto">
            {/* Header */}
            <div className="flex items-center justify-between p-4 backdrop-blur-md bg-white/30 border-b border-white/20 shadow-sm">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    },
                    {
                      value: "in-stock",
                      label: "In Stock",
                      count: products.filter((p) => p.stock_quantity > 0)
                        .length,
                    },
                    {
                      value: "out-of-stock",
            ...
            {/* Footer Actions */}
            <div className="fixed bottom-0 left-0 right-0 p-4 border-t border-white/20 backdrop-blur-md bg-white/30 flex gap-3 shadow-md z-50">
              <button
                onClick={() => {
                  clearAllFilters();
